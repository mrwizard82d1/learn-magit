...
(defn typical-stage-separation
  "Calculate a typical stage separation (from bottom of previous stage to top of next stage)."
  ([length-unit]
   (case length-unit
  ...
  ([qualifier reference-time]
   (let [completion-time-in-seconds (typical-stage-completion-time)
     ...
     (case qualifier
       ...
       :starting-after (let [previous-java-stop-time java-reference-time
                             java-changeover
                             (Duration/ofSeconds (Math/round (typical-stage-completion-changeover)))
                             java-start-time (.addTo java-changeover previous-java-stop-time)
                             java-stop-time (.addTo java-completion-time java-start-time)
